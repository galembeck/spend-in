...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <Link
          className="relative px-4 py-2 text-white/90 hover:text-zinc-600 dark:text-neutral-300"
          href={item.link}
          key={`link-${
        ...
        >
          ...
          <span className="relative z-20 font-medium text-base hover:text-black">
            {item.name}
          </span>
        </Link>
      ))}
...
export const NavbarLogo = () => {
  return (
    <Link
      ...
      href="#"
    >
      <Image alt="logo" height={32} src="/assets/icons/logo.svg" width={32} />
      <span className="font-bold text-2xl text-primary-light-100">
        Spend.In
      </span>
    </Link>
  );
